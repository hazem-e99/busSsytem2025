...
import { CheckCircle, CreditCard, Banknote, Crown, Shield, Bell, Smartphone, Wallet } from 'lucide-react';
...
export default function StudentSubscriptionPage() {
    ...
    try {
      ...
      if (res?.success) {
        ...
        showToast({
          type: 'success',
          title: 'Success!',
          message: `Payment ${paymentMethod === PaymentMethod.Online ? 'submitted successfully' : 'pending admin approval'} for ${selectedPlan.name} plan! You have been ${actionText}.`
        });
  ...
  return (
    <div className="p-6 space-y-8">
          ...
          <CardContent className="relative z-10 pt-0">
            {status === 'active' && (activeSubscription || currentPlanDetails) ? (
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-lg font-semibold text-green-600">
                            ${(activeSubscription?.subscriptionPlanPrice || currentPlanDetails?.price)?.toFixed(2) || '—'}
                   </div>
              </div>
              <div>
                          <div className="text-sm text-gray-500 mb-1">Duration</div>
                          <div className="text-lg font-semibold text-gray-900">
                            {activeSubscription?.durationInDays || currentPlanDetails?.durationInDays || '—'} days
                          </div>
                        </div>
                 </div>
                   </div>
              ...
              <div>
                        ...
                        <p className="text-sm text-gray-600">Payment method & reference</p>
                      </div>
                   </div>
                    <div className="space-y-3">
                      <div>
              ...
              <div>
                          <div className="text-sm text-gray-500 mb-1">Reference Code</div>
                  ...
                  <CheckCircle className="w-5 h-5 text-green-600" />
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">Subscription timeline</p>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <div>
                ...
                {activeSubscription && (
                  <div className="bg-white/70 backdrop-blur-sm rounded-xl p-6 border border-white/50">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="font-semibold text-gray-900">Subscription Progress</h3>
                      <span className="text-sm text-gray-600">
                        {Math.round(((new Date().getTime() - new Date(activeSubscription.startDate).getTime()) / 
                        (new Date(activeSubscription.endDate).getTime() - new Date(activeSubscription.startDate).getTime())) * 100)}% Used
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between text-xs text-gray-500 mt-2">
                      <span>Start: {new Date(activeSubscription.startDate).toLocaleDateString()}</span>
                      <span>End: {new Date(activeSubscription.endDate).toLocaleDateString()}</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 mb-6 max-w-md mx-auto">
                  You don&apos;t have an active subscription plan. Choose a plan below to get started with our bus service.
                </p>
                  ...
                  <div className="bg-white/70 backdrop-blur-sm rounded-lg p-4 border border-white/50">
                    ...
                    <div className="text-lg font-semibold text-gray-900">
                      {lastSubscriptionPayment?.createdAt ? 
                        new Date(lastSubscriptionPayment.createdAt).toLocaleDateString() : 'Never'}
                    </div>
                  </div>
                  <div className="bg-white/70 backdrop-blur-sm rounded-lg p-4 border border-white/50">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm text-text-muted">
              {status === 'active' ? 'Upgrade your current plan' : 'Select a plan and proceed to payment'}
            </span>
          </div>
        </div>
      ...
      <Modal isOpen={methodModalOpen} onClose={() => setMethodModalOpen(false)} title="Payment Method" size="md">
            ...
            <div className="bg-primary/5 p-4 rounded-lg border border-primary/20">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-600">{selectedPlan.duration || 'One term'}</p>
                  {activeSubscription && selectedPlan.id !== activeSubscription.subscriptionPlanId && (
                    <div className="flex items-center gap-1 mt-1">
                      <Crown className="w-3 h-3 text-purple-600" />
                      <span className="text-xs text-purple-600 font-medium">Upgrade from {activeSubscription.subscriptionPlanName}</span>
                    </div>
                  )}
                </div>
                <div className="text-right">
                  ...
                  {activeSubscription && selectedPlan.id !== activeSubscription.subscriptionPlanId && (
                    <div className="text-xs text-gray-500 mt-1">
                      Current: ${activeSubscription.subscriptionPlanPrice?.toFixed(2)}
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                <div className="space-y-2">
                  ...
                  <div>
                    <div className="font-semibold text-gray-900 text-sm">InstaPay</div>
              ...
              >
                <div className="space-y-2">
                  ...
                  <div>
                    <div className="font-semibold text-gray-900 text-sm">Offline</div>
          ...
          {paymentMethod === PaymentMethod.Online && (
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h5 className="font-semibold text-blue-900">Bank Transfer Info</h5>
              </div>
              <div className="space-y-2">
                ...
                <div className="bg-amber-50 border border-amber-200 p-2 rounded text-xs text-amber-800">
                  <p className="font-medium">⚠️ Transfer ${selectedPlan?.price.toFixed(2)}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {paymentMethod === PaymentMethod.Online && (
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700">
                Payment Reference Code <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
              ...
              {paymentMethod === PaymentMethod.Online && !paymentReferenceCode.trim() && (
                <p className="text-xs text-red-500">
                  Reference code is required for InstaPay transactions
                </p>
              )}
            </div>
          )}
      ...
      <Modal isOpen={pendingModalOpen} onClose={() => setPendingModalOpen(false)} title="Payment Pending Approval" size="md">
        <div className="space-y-4">
          <div className="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
            <Bell className="w-6 h-6 text-amber-600" />
            <div>
              <p className="font-semibold text-amber-800">Payment Submitted Successfully!</p>
              <p className="text-sm text-amber-700">
                Your offline payment is pending admin approval. You will be notified once it&apos;s confirmed.
              </p>
            </div>
          </div>
          <div className="space-y-2 text-sm text-gray-600">
            <p>• Your payment will be reviewed by an administrator</p>
            <p>• You&apos;ll receive a notification when approved</p>
            <p>• Check your payment status in the dashboard</p>
          </div>
      ...
      {/* Upgrade Plan Modal */}
      <Modal isOpen={upgradeModalOpen} onClose={() => setUpgradeModalOpen(false)} title="Upgrade Your Plan" size="lg">
        <div className="space-y-6">
          {/* Current Plan Info */}
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-xl border border-green-200">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-sm text-green-600">Your active subscription</p>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <div className="text-sm text-green-600 mb-1">Plan Name</div>
                <div className="font-semibold text-green-800">
                  {activeSubscription?.subscriptionPlanName || currentPlan || '—'}
                </div>
              </div>
              <div>
                <div className="text-sm text-green-600 mb-1">Price</div>
                <div className="font-semibold text-green-800">
                  ${(activeSubscription?.subscriptionPlanPrice || currentPlanDetails?.price)?.toFixed(2) || '—'}
                </div>
              </div>
          ...
          <div>
            ...
            {availablePlans.length > 0 ? (
              ...
              <div className="text-center py-8">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">No Upgrade Options Available</h3>
                <p className="text-gray-600">
                  You&apos;re already on the highest plan available, or no upgrade options are currently available.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-blue-50 p-4 rounded-xl border border-blue-200">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h3 className="font-semibold text-blue-800">Upgrade Benefits</h3>
                <p className="text-sm text-blue-600">What happens when you upgrade</p>
              </div>
            </div>
            <div className="space-y-2 text-sm text-blue-700">
              <p>• Your new plan will be activated immediately after payment confirmation</p>
              <p>• You&apos;ll get access to all features of the upgraded plan</p>
              <p>• The upgrade will extend your subscription duration</p>
              <p>• You can continue using your current plan until the upgrade is processed</p>
            </div>
          </div>
