...
const TableFooter = forwardRef<HTMLTableSectionElement, React.HTMLAttributes<HTMLTableSectionElement>>(
  ({ className, ...props }, ref) => (
    <tfoot
      ref={ref}
      className={cn('bg-card-hover font-medium [&>tr]:last:border-b-0', className)}
      {...props}
    />
  )
);
...
const TableRow = forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn(
        'border-b border-border transition-colors duration-200 hover:bg-card-hover data-[state=selected]:bg-primary-light/20',
        className
      )}
      {...props}
    />
...
const TableHead = forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        'h-12 px-4 text-start align-middle font-semibold text-text-secondary [&:has([role=checkbox])]:pr-0',
        className
      )}
      {...props}
    />
